...
class Sear {
  constructor(options) {
    ...
    this._raw = { ...options.data, ...this.retrieve(options.persist) };
...
Supported data types are: (Basic) Objects, Functions, Arrays, Dates, Strings, Numbers, and Booleans.\n`
  ...
  persist(ident, store) {
    ...
    return (localStorage[ident] = JSON.stringify({
      data,
      computed
    }));
  }
  ...
  notify(signal) {
    ...
    while (signal.length > 0) {
      ...
      if (!this._signals[current] || this._signals[current].length < 1) return;
      this._signals[current].forEach(handler =>
        handler(
          this.clone(
            this.get(
              typeof raw === 'function' ? this._proxied : this._cache,
              current
            )
          )
        )
    ...
    return {
      get(obj, prop) {
        ...
        if (typeof obj[prop] === 'function') {
          ...
          parent.observe(id, () => parent.set(parent._cache, id, undefined));
